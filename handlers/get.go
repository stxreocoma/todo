...
func GetTasks(c *fiber.Ctx) error {
	...
	if c.Query("search") == "" {
		...
		if result.Error != nil {
			return c.Status(fiber.StatusBadRequest).JSON(map[string]any{"Error": models.ErrRepeat})
		}
	} else {
		...
		if err != nil {
			...
			if result.Error != nil {
				return c.Status(fiber.StatusInternalServerError).JSON(map[string]any{"Error": models.ErrSearch})
			}
		} else {
...
func GetTask(c *fiber.Ctx) error {
	...
	if c.Query("id") == "" {
		return c.Status(fiber.StatusBadRequest).JSON(map[string]any{"Error": models.ErrID})
	}
